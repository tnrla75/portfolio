...
$(document).ready(function() {
	...
	<% if( seat.equals("이코노미 클래스")){ %>
		$('#seatGrade2').val('이코노미 클래스').prop("selected",true);
	<%}else if(seat.equals("비지니스 클래스")){%>
		$('#seatGrade2').val('비지니스 클래스').prop("selected",true);
	<%}else if(seat.equals("퍼스트 클래스")){%>
		$('#seatGrade2').val('퍼스트 클래스').prop("selected",true);	
	<%}%> --%>
});	
	...
	<div id="textdiv">
								...
								<section id="rauchbier" class="tab-panel">
									<div class="searchBot">
										<div>
											<section class="airplainchoice">
											<img src="../img/icon/take-off.png" width="20" height="20" class="flighticon"> 
												<input type="text" placeholder="출발지" class="departure" value="<%= request.getParameter("departure2") %>" name='departure2'>
											</section>
											<section class="airplainchoice">
											<img src="../img/icon/landing.png" width="20" height="20" class="flighticon"> 
												<input type="text" placeholder="도착지" class="arrive" value="<%= request.getParameter("arrive2") %>" name='arrive2'>
		...
		<div id="flightdivmain">
			...
			<tr class="tr1">
				...
				<% 
				String flightimg1 = "";
				if(ticketBeanList3.get(i).getFlight_name().equals("제주")){ 
					flightimg1 = "jejuair.png";
				}else if(ticketBeanList3.get(i).getFlight_name().equals("아시아나")){
				...
				<td class="td1"><span class="airport">소요시간</span><br><img src="aaa.jpg"></td>
				...
				<td class="td1" rowspan="2"><fmt:formatNumber value="<%= ticketBeanList3.get(i).getFlight_Ticket_Price() %>" groupingUsed="true" />원<input type="button" value="선택" class="choicebtn"></td>
			</tr>
		...
		[이전]&nbsp;
		<%}else{ %>
		<a href="flightTicketSearch_oneway.air?page3=<%=nowPage3-1 %>&departure2=<%= request.getParameter("departure2") %>&arrive2=<%= request.getParameter("arrive2") %>&departureDay2=<%= request.getParameter("departureDay2") %>">[이전]</a>&nbsp;
		<%} %>
		...
		[다음]
		<%}else{ %>
		<a href="flightTicketSearch_oneway.air?page3=<%=nowPage3+1 %>&departure2=<%= request.getParameter("departure2") %>&arrive2=<%= request.getParameter("arrive2") %>&departureDay2=<%= request.getParameter("departureDay2") %>">[다음]</a>
		<%} %>
	</section>
