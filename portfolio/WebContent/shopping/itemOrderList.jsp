...
<body>
	<form action="itemBuyList.dutyfree?command=buyList" method="post" onsubmit="return check()">
		<%
			...
			String [] item_count = request.getParameterValues("count"); 
			String [] option = request.getParameterValues("option"); 
			
			ArrayList<String> count_arr = new ArrayList<String>();
			ArrayList<String> option_arr = new ArrayList<String>();
			
			/* session.setAttribute("item", item);
			session.setAttribute("count_arr", count_arr);
			session.setAttribute("option_arr", option_arr); */
		
			
			for(int j=0; j<item_count.length; j++) {
				count_arr.add(item_count[j]);
				if (item_count[j] == "") {
					count_arr.remove("");
				}
			}
			for(int j=0; j<option.length; j++) {
				option_arr.add(option[j]);
				if (option[j] == "") {
					option_arr.remove("");
				}
			}
		...
		<section>
				...
				<div id="result">
					...
					<% 
						...
						for (int i=0; i<item.size(); i++) {
							double discount = (double)item.get(i).getDiscount()/(double)100;
							int count = Integer.parseInt(count_arr.get(i));
							totalWon += count*item.get(i).getItemWon();
							totalDol += count*item.get(i).getItemDollar();
							discountWon += count*item.get(i).getDiscountWon();
							discountDol += count*item.get(i).getItemDollar()*(1-discount);
					%>
					<% } %>
					<div class="price">
						...
						<div>
							<p>$<%= totalDol %></p>
							<p><fmt:formatNumber value="<%= totalWon %>" groupingUsed="true" />원</p>
						</div>
					</div>
					<div class="price">
						<span>총 할인 금액</span>
						<div>	
							<p>$<fmt:formatNumber value='<%= totalDol-discountDol %>' pattern='.0' /></p>
							<p><fmt:formatNumber value="<%= totalWon-discountWon %>" groupingUsed="true" />원</p>
						</div>
					</div>
					<div class="finish">
						<span>최종 결제 금액</span>
						<div>
							<p>$<fmt:formatNumber value='<%= discountDol %>' pattern='.0' /></p>
							<p><fmt:formatNumber value="<%= discountWon %>" groupingUsed="true" />원</p>
						</div>
					</div>
			...
			<table align="center" cellpadding="10px" class="order_info">
				...
				<% for (int i=0; i<item.size(); i++) {
					...
					int count = Integer.parseInt(count_arr.get(i));
				%>
				<input type="hidden" value="<%=item.get(i).getItemCode()%>" name="itemCode">
				<input type="hidden" value="<%=option_arr.get(i)%>" name="option">
				<input type="hidden" value="<%=count_arr.get(i)%>" name="count">
				<tr class="tableLine" align="center">
					<td id="itemDetail">
						<div><img src="<%= item.get(i).getItemImg() %>" width="230px" height="240px"></div>
						<div>
							<p><%= item.get(i).getBrandName() %></p>
							<p><%= item.get(i).getItemName() %></p>
							<p>옵션 : <%= option_arr.get(i) %></p>
						</div>
					</td>
					<td><%= count_arr.get(i) %></td>
					<td>
						<p>$<%= item.get(i).getItemDollar() %></p>
						<p><fmt:formatNumber value="<%= item.get(i).getItemWon() %>" groupingUsed="true" />원</p>
					</td>
					<td>
						<p>$<%= count*item.get(i).getItemDollar() %></p>
						<p><fmt:formatNumber value="<%= count*item.get(i).getItemWon() %>" groupingUsed="true" />원</p>
					</td>
					<td>
						<p>$<fmt:formatNumber value='<%= count*item.get(i).getItemDollar()*discount %>' pattern='.0' /></p>
						<p><fmt:formatNumber value="<%= count*item.get(i).getItemWon()-count*item.get(i).getDiscountWon() %>" groupingUsed="true" />원</p>	
					</td>
				</tr>
				<% } %>	
			</table>
